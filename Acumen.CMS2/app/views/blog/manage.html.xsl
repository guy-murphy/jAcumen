...
>
    ...
    <h3>Manage Blogs</h3>
    <xsl:apply-templates select="$view-state/item[@name='blogs']/list" mode="blog" />
      ...
      <table>
        <tr>
          ...
          <td class="help" rowspan="2">
            <p>
              If you are not sure what ID to give the blog entry then
    ...
    <h3>Link Topic As Blog</h3>
    <form method="post" action="/blog/link">
      <input type="hidden" name="id" value="{$topic-id}" />
      <table class="metadata">
              ...
              this one as a blog. The text input box will autocomplete
    ...
    <p>
      A <em>blog</em> is simply one page attached to the other with a
      link where the link is of type blog. This 'other' page
      is simply displayed along with the main page as a blog.
    </p>
    
    <p>
      If you take a look the <a href="/association/view/{$topic-id}">links</a>
      for a topic with blogs attached to it, you will see those links 
      that are of type blog.
    </p>
      
	</xsl:template>
	
	<xsl:template match="item[@name='blogs']/list" mode="blog">
	  <table>
	    <thead>
	      <th>blog entry</th><th>created on</th><th>created by</th><th>&#32;</th>
	    </thead>
	    <tbody>
  	    <xsl:apply-templates select="topic" mode="blog">
	...
	<xsl:template match="item[@name='blogs']/list/topic" mode="blog">
	  <tr>
	    <td>
	    ...
	    <td>
	      <a href="/blog/delink?id={$topic-id}&amp;from={@id}">delink</a>
	    </td>
	  </tr>
	</xsl:template>
